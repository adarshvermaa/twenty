...
fetch(`http://3.225.218.18:5086/api/products/categories2/${id2}`)
  ...
  .then(data => {
    if (data && data.Data.length >= 0) {
      const noiseData = data.Data.map((data) => {
        return {
          id: data._id,
          img: `http://3.225.218.18:5086/api/static/${data.image}`,
          name: data.title.en,
...
fetch(`http://3.225.218.18:5086/api/products/categories3/${id3}`)
  .then(res => { return res.json() })
  .then(data => {
    if (data && data.Data.length >= 0) {
      const noiseData = data.Data.map((data) => {
...
fetch(`http://3.225.218.18:5086/api/products/categories/${id}`)
  .then(res => { return res.json() })
  .then(data => {
    if (data && data.Data.length >= 0) {
      const noiseData = data.Data.map((data) => {
